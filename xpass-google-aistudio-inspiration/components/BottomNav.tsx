...
const BottomNav: React.FC<BottomNavProps> = ({ currentTab, onTabChange }) => {
  const navItems: { id: Tab; icon: React.ElementType; label: string }[] = [
    ...
    { id: 'wellness', icon: Activity, label: 'My Plan' },
    { id: 'credits', icon: Zap, label: 'Credits' },
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className="flex flex-col items-center justify-center w-full group relative py-1"
            >
              <div className={`transition-all duration-300 transform ${isActive ? 'text-brand-500 -translate-y-1' : 'text-zinc-600 group-hover:text-zinc-400'}`}>
                <Icon size={22} strokeWidth={isActive ? 2.5 : 1.5} className={isActive ? 'drop-shadow-[0_0_8px_rgba(255,82,0,0.6)]' : ''} />
              </div>
              
              <span className={`text-[9px] font-mono mt-1 uppercase tracking-wider transition-colors duration-300 ${isActive ? 'text-white' : 'text-transparent group-hover:text-zinc-600'}`}>
                {item.label}
              </span>
