...
import { Bot, X, Send, MapPin, Camera, Image as ImageIcon, Sparkles, ChevronRight, ClipboardList } from 'lucide-react';
...
const AICoach: React.FC = () => {
  ...
  // State for Anamnesis Flow
  const [anamnesisStep, setAnamnesisStep] = useState<number>(0);
  
  const [messages, setMessages] = useState<Message[]>([
    { id: '1', sender: 'bot', type: 'text', text: 'Olá! Sou seu XPASS Coach com IA. Posso encontrar academias, analisar seus equipamentos ou criar um plano personalizado para você.' }
  ]);
  ...
  const startAnamnesis = () => {
    ...
    setMessages(prev => [...prev, {
       ...
       sender: 'bot',
       type: 'text',
       text: 'Vamos criar seu plano perfeito. Primeiro, qual é o seu principal objetivo hoje? (Ex: Hipertrofia, Emagrecimento, Resistência)'
    }]);
  };
  ...
  const processAnamnesis = (input: string) => {
     setTimeout(() => {
        let responseText = '';
        if (anamnesisStep === 1) {
           responseText = 'Ótimo objetivo. Qual é o seu peso atual (kg) e altura (cm)?';
           setAnamnesisStep(2);
        } else if (anamnesisStep === 2) {
           responseText = 'Entendido. Quantas vezes por semana você pretende treinar?';
           setAnamnesisStep(3);
        } else if (anamnesisStep === 3) {
           responseText = 'Perfeito! Processei seus dados. Criei um plano de treino "Full Body" e uma meta de 2400kcal. Acesse a aba "My Plan" para ver os detalhes!';
           setAnamnesisStep(0); // End flow
        }
  ...
  const processAIResponse = (input: string) => {
    setTimeout(() => {
      ...
      // SCENARIO 2: Create Plan
      else if (lowerInput.includes('plano') || lowerInput.includes('dieta') || lowerInput.includes('ficha')) {
        startAnamnesis();
        setIsTyping(false);
        return;
      }
      ...
      >
        ...
        <div className="p-3 bg-onyx-900 border-t border-white/5 flex flex-col gap-2">
           ...
           <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
              ...
              >
                 <ClipboardList size={12} className="inline mr-1 text-purple-500" /> Criar Plano
              </button>
              <button 
           ...
           <div className="flex gap-2">
              <input 
                ...
                placeholder={anamnesisStep > 0 ? "Respondendo Coach..." : "Pergunte ao Gemini..."}
