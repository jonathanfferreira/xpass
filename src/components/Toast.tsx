...
const ToastItem: React.FC<{ toast: ToastMessage; onRemove: () => void }> = ({ toast, onRemove }) => {
  ...
  const styles = {
    ...
    info: 'bg-onyx-900 border-brand-500/50 text-white shadow-[0_0_20px_rgba(255,82,0,0.2)]',
  };
  ...
  return (
    <div className={`pointer-events-auto flex items-start gap-3 p-4 rounded-xl border w-80 animate-in slide-in-from-right duration-300 relative overflow-hidden group ${styles[toast.type]}`}>
      ...
      <button onClick={onRemove} className="text-zinc-500 hover:text-white transition-colors cursor-pointer">
        <X size={16} />
      </button>
      
      {/* Progress Bar */}
      <div className="absolute bottom-0 left-0 h-0.5 bg-current opacity-30 w-full animate-pulse" />
    </div>
  );
};
